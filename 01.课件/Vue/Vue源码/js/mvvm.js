function MVVM(options) {
  ...
  */
  ...
  /*
    问题1:请问Vue1更新数据是同步更新还是异步更新?
    回答:同步更新

    问题2:请问Vue2更新数据是同步更新还是异步更新?
    回答:同步更新
  
    问题3:请问Vue1更新DOM是同步更新还是异步更新?
    回答:同步更新

    问题4:请问Vue2更新DOM是同步更新还是异步更新?
    回答:异步更新,Vue2故意将更新DOM的方法交给nextTick执行,才会导致异步更新DOM的效果

      无论是Vue1还是Vue2,渲染机制都是浏览器的东西,由GUI线程负责,GUI渲染页面一定是异步的
    ...
    回答:组件为单位更新(范围更新)
      ...
      从表面上观察,Vue1的精准更新方案会胜于Vue2的范围更新
        Vue2其实没有想象的那么差,因为Vue2引进了虚拟DOM和diff算法
          他会去找到没有变化的节点,防止他被误杀

      实际上Vue1耗费的内存会比Vue2高很多

      在Vue1中,1000个插值语法会生成1000个的watcher对象
      在Vue2中,1个组件会生成1个watcher对象

    问题7:请问Vue1通过数组的下标操作数组的数据,是否具有响应式效果?
    回答:Vue1中数组的每个下标都是响应式的,操作他们具有响应式效果

    问题8:请问Vue2通过数组的下标操作数组的数据,是否具有响应式效果?
    回答:Vue2中数组的下标不是响应式的,操作没有响应式效果

      问题:为什么Vue2中不会数组的下标进行响应式处理
      回答:这是尤大大故意的,每创建一个响应式属性就必然产生一个dep对象
        由于项目中,很少会直接通过数组的下标操作数据,为每个下标生成dep对象太浪费性能了

      问题:在Vue2中,如何操作数组的内容才能具有响应式效果?
      回答:可以通过Vue2重写数组的7种方法来解决
          这七种方法都具有响应式效果,分别是shift,unshift,pop,push,sort,splice,reverse

      问题:Vue2是怎么做到不对数组的下标进行数据劫持的?
      回答:Vue2通过Array.isArray判断数据是不是数组,如果是数组,就使用for循环跳过对其下标的数据劫持
          将数组内部的所有属性值进行数据劫持
            所以数组的下标不是响应式的,但是数组中如果存的是对象,那么这些对象的属性就会是响应式的

      问题:Vue2是如何区分data中的数组,和非data中的数组两者,并实现他们的七种方法的效果不同的
      回答:
        1.Vue2会创建一个全新的对象,该对象的__proto__指向Array.prototype
        2.Vue2会在流程1中创建的对象身上,书写数组同名的7种方法
        3.在数据劫持的时候,如果出现了数组,那么该数组的__proto__就会被修改成流程1中创建的对象
  */
}
