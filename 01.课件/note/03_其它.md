...
- 2XX: 表示成功处理请求
  - 200->一般代表完全成功,可以获取到数据
  - 201->一般代表请求成功,但是参数格式有问题
- 3XX: 需要重定向, 浏览器直接跳转
  - 302->代表地址重定向,我们请求服务器的A资源,服务器通过响应,将浏览器地址重定向到B地址去
  - 304->代表协商缓存,我们第二次请求服务器的A资源,服务器发现浏览器内存中还存有该文件,那么就不返回该资源,会通过响应通知浏览器,使用内存中上次请求的资源
- 4XX: 客户端请求错误
  - 403->代表有该资源,但是由于你权限不够,拒绝给你
    - 问题:只要前端的token之类的数据没传错,大概率是后端问题
  - 404->代表服务器没有请求的该资源
    - 问题:大概率是前端路径写错
- 5XX: 服务器端错误
  - 500->代表服务器崩溃,无法处理该请求
  - 501->代表本次请求服务器无法解决,请求方式有问题

#### 区别ajax请求与一般HTTP请求

ajax不是一种特殊的请求,他是一种发送HTTP请求的特殊方式
...
1. 配置通用的基础路径和超时时间
   ...
   2. **超时时间可以延伸到请求最大并发数以及超时的作用**
      1. **小程序中最多同时并发10个请求,Chrome和Firefox最多并发6个请求**
      2. **小程序最大超时时间和默认超时时间都是60s,Chrome和Firefox我们配置是20s**
         1. **他们会等待请求一段时间,如果在指定时间之内,没有响应,那么当前请求自动变为404,视为失败**
2. 显示请求进度条
   1. 显示进度条: 请求拦截器回调
   2. 结束进度条: 响应拦截器回调
...
7. 对token过期的错误进行处理
   1. **token是否过期,是每次浏览器发token给服务器的时候,服务器会解密得到创建时间,进行比较,如果过期了服务器会返回响应码401通知前端**
...
- layout 布局(排版)：计算元素的位置和大小信息
  - 重排:根据最新的位置信息,重新对页面的布局进行排版
- render 渲染：将颜色/文字/图片等渲染上去
  - 重绘:根据最新的内容,渲染最新的结果到页面上给用户观看
  - 重排一定伴随着重绘,重绘不一定伴随着重排
  - **扩展:重绘的范围是整个图层**
    - **性能优化:如果知道某个元素会频繁发生变化,需要频繁渲染,可以给他单独开启一个图层**
      - **开启图层的最快方法:加css样式->will-change:true**
